...
{
	uint32_t start=__HAL_TIM_GetCounter(&htim7);
	while((__HAL_TIM_GetCounter(&htim7)-start)<time)
	{
...
//	printf("Pin is in output mode now\n");
//}
//
...
//	HAL_GPIO_Init(GPIOx, &GPIO_InitStruct);
//	printf("Pin is in input mode now\n");
//}
...
//	  	  	  	  		  char sleep_msg[] = "MCU is going to stop mode...\r\n";
...
//	  	  	  	  		  	  	      // MCU will wake up here after 10 seconds
//	  	  	  	  		  	  	      SystemClock_Config();
//	  	  	  	  		  	  	      HAL_ResumeTick();
//
//	  	  	  	  		  	  	      // Indicate the MCU has woken up
//	  	  	  	  		  	  	      char wakeup_msg[] = "MCU woke up from stop...\r\n";
//	  	  	  	  		  	  	      HAL_UART_Transmit(&huart2, (uint8_t *)wakeup_msg, strlen(wakeup_msg), HAL_MAX_DELAY);/* USER CODE BEGIN 3 */
	  active_time_start = HAL_GetTick();
	  if(DHT22_Start())
	  	     {
...
//	  	  	  	  	  		  	  	  HAL_UART_Transmit(&huart2, (uint8_t *)time, sizeof(time), 300);
	  ...
	  active_time = (HAL_GetTick() - active_time_start);
	  char buffer[50];
	  	      sprintf(buffer, "Active time: %lu ms\r\n", active_time);
	  	      HAL_UART_Transmit(&huart2, (uint8_t *)buffer, strlen(buffer), HAL_MAX_DELAY);
	  	      HAL_Delay(1000); // Delay to avoid immediate sleep cycle
...
//    HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
    ...
    // Calculate the next alarm time (10 seconds later)
    sAlarm.AlarmTime.Seconds += 25;
    ...
    HAL_RTC_GetTime(hrtc, &sTime, RTC_FORMAT_BIN);
        ...
        // Format and transmit the date and time
        sprintf(date, "Date: %02d.%02d.%02d\t", sDate.Date, sDate.Month, sDate.Year);
        sprintf(time, "Time: %02d.%02d.%02d\r\n", sTime.Hours, sTime.Minutes, sTime.Seconds);
        HAL_UART_Transmit(&huart2, (uint8_t *)date, sizeof(date), 300);
        HAL_UART_Transmit(&huart2, (uint8_t *)time, sizeof(time), 300);
...
//        //HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
//
//	// Toggle LED to indicate awake state
////	  HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
////
////	  // Notify PC
////	  char sleep_msg[] = "MCU is going to sleep...\r\n";
////	  HAL_UART_Transmit(&huart2, (uint8_t *)sleep_msg, sizeof(sleep_msg), HAL_MAX_DELAY);
////
////	  // Enter sleep mode
